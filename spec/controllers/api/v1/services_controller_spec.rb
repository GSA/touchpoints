  ...
  describe 'authenticated request, using HTTP_AUTH' do
    ...
    describe '#index' do
      context 'passing a valid API_KEY' do
        ...
        let!(:service1) { FactoryBot.create(:service, organization: user.organization, service_provider:, service_owner_id: user.id) }
        let!(:service2) { FactoryBot.create(:service, organization: user.organization, service_provider:, service_owner_id: user.id) }
        let!(:service3) { FactoryBot.create(:service, organization: user.organization, service_provider:, service_owner_id: user.id) }
        ...
        it 'return an array of published service providers' do
          ...
          expect(@parsed_response['data'].first['type']).to eq("services")
          expect(@parsed_response['data'].first['id']).to eq(service1.id.to_s)
        end
      end
    end
  end
