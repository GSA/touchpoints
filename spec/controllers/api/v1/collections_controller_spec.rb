  ...
  describe 'authenticated request, using HTTP_AUTH' do
    ...
    describe '#index' do
      context 'passing a valid API_KEY' do
        ...
        let!(:collection1) { FactoryBot.create(:collection, organization: user.organization, aasm_state: :published) }
        let!(:collection2) { FactoryBot.create(:collection, organization: user.organization, aasm_state: :published) }
        ...
        it 'return an array of published service providers' do
          ...
          expect(@parsed_response['data'].first["type"]).to eq("collections")
