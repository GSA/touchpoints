    ...
    describe 'without passing ?API_KEY' do
      before do
        request.env['HTTP_AUTHORIZATION'] =
          ActionController::HttpAuthentication::Basic.encode_credentials(ENV.fetch('API_HTTP_USERNAME'),
                                                                         ENV.fetch('API_HTTP_PASSWORD'))
      ...
      context 'date_filter' do
        ...
        it 'returns an array of forms with submissions which occurred during the past day' do
          get :show, format: :json,
                     params: { id: form.short_uuid, 'API_KEY' => user.api_key, start_date: Date.today.strftime('%Y-%m-%d'), end_date: 1.day.from_now.strftime('%Y-%m-%d') }
          parsed_response = JSON.parse(response.body)
          expect(response.status).to eq(200)
          expect(parsed_response['data']['relationships']['submissions']['data'].size).to eq(1)
        end
        ...
        it 'returns an array of forms with submissions which occurred during the past two days' do
          get :show, format: :json,
                     params: { id: form.short_uuid, 'API_KEY' => user.api_key, start_date: 1.day.ago.strftime('%Y-%m-%d'), end_date: 1.day.from_now.strftime('%Y-%m-%d') }
          parsed_response = JSON.parse(response.body)
          expect(response.status).to eq(200)
          expect(parsed_response['data']['relationships']['submissions']['data'].size).to eq(2)
        end
        ...
        it 'returns an array of forms with submissions which occurred during the past week' do
          get :show, format: :json,
                     params: { id: form.short_uuid, 'API_KEY' => user.api_key, start_date: 7.days.ago.strftime('%Y-%m-%d'), end_date: 1.day.from_now.strftime('%Y-%m-%d') }
          parsed_response = JSON.parse(response.body)
          expect(response.status).to eq(200)
          expect(parsed_response['data']['relationships']['submissions']['data'].size).to eq(3)
        end
        ...
        it 'returns an array of forms with submissions which occurred during the past week with paging' do
          get :show, format: :json,
                     params: { id: form.short_uuid, 'API_KEY' => user.api_key, page: 0, size: 2, start_date: 7.days.ago.strftime('%Y-%m-%d'), end_date: 1.day.from_now.strftime('%Y-%m-%d') }
          parsed_response = JSON.parse(response.body)
          expect(response.status).to eq(200)
          expect(parsed_response['data']['relationships']['submissions']['data'].size).to eq(2)
        end
        ...
        it 'returns an invalid input response for a bad date input' do
          get :show, format: :json,
                     params: { id: form.short_uuid, 'API_KEY' => user.api_key, start_date: 'Foo', end_date: 7.days.from_now.strftime('%Y-%m-%d') }
          parsed_response = JSON.parse(response.body)
          expect(response.status).to eq(400)
        end
      end
