...
describe Api::V1::PersonasController, type: :controller do
  ...
  describe 'authenticated request, using HTTP_AUTH' do
    ...
    describe '#index' do
      context 'passing a valid API_KEY' do
        ...
        let!(:persona1) { FactoryBot.create(:persona) }
        let!(:persona2) { FactoryBot.create(:persona) }
        ...
        it 'return an array of published service providers' do
          ...
          expect(@parsed_response['data'].first["type"]).to eq("personas")
          expect(@parsed_response['data'].first['id']).to eq(persona1.id.to_s)
        end
      end
    end
  end
