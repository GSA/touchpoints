...
RSpec.describe '/omb_cx_reporting_collections', js: true do
  ...
  let!(:service) do
    FactoryBot.create(:service, organization:, service_provider:, service_owner_id: user.id)
  end
  let!(:collection) do
    FactoryBot.create(:collection, organization:, user:, service_provider:)
  end
  let(:omb_cx_reporting_collection) do
    FactoryBot.create(:omb_cx_reporting_collection, collection:, service:)
  end
