...
feature 'Data Collections', js: true do
  ...
  let!(:another_service_provider) do
    FactoryBot.create(:service_provider, organization: another_organization, name: 'Another HISP')
  end
  ...
  let!(:service) do
    FactoryBot.create(:service, organization:, service_provider:, hisp: true, service_owner_id: user.id)
  end
  let!(:collection) do
    FactoryBot.create(:collection, organization: another_organization, user:,
                                   service_provider: another_service_provider)
  end
  let!(:admin_collection) do
    FactoryBot.create(:collection, organization:, user: admin, service_provider:)
  end
  ...
  context 'as an Admin' do
    ...
    describe 'GET /index' do
      let!(:collection_1) do
        FactoryBot.create(:collection, organization:, user: admin, service_provider:, year: 2021, quarter: 2)
      end
      let!(:collection_2) do
        FactoryBot.create(:collection, organization:, user: admin, service_provider:, year: 2022, quarter: 1)
      end
      let!(:collection_3) do
        FactoryBot.create(:collection, organization:, user: admin, service_provider:, year: 2022, quarter: 1)
      end
