...
RSpec.describe Collection, type: :model do
  ...
  describe 'a minimally valid Collection' do
    before do
      @collection = Collection.create(name: 'Test Collection', organization:, user:,
                                      service_provider:, year: 2022, quarter: 2)
    end
  ...
  describe 'reflection text that is too long' do
    context do
      before do
        ...
        @collection = Collection.create(name: 'Test Collection', organization:, user:,
                                        service_provider:, year: 2022, quarter: 2, reflection: reflection_text)
      end
