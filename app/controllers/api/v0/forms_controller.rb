    ...
    class FormsController < ::ApiController
      ...
      def show
        ...
        begin
          ...
          render json: { error: { message: "invalid date format, should be 'YYYY-MM-DD'", status: 400 } },
        ...
        respond_to do |format|
          format.json do
            if form
              render json: {
                form:,
                responses: form.submissions.where('created_at BETWEEN ? AND ?', start_date,
                                                  end_date).limit(size).offset(size * page),
                links: links(form, page, size)
              }
            else
              render json: { error: { message: "no form with Short UUID of #{params[:id]}", status: 404 } },
                     status: :not_found
            end
          end
        end
