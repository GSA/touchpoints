...
module Admin
  class ServicesController < AdminController
    ...
    def index
      ...
      @services = if service_manager_permissions?
                    if tag_name.present?
                      Service.tagged_with(tag_name).includes(:organization).order('organizations.name', :name)
                    else
                      Service.all.includes(:organization).order('organizations.name', :name)
                    end
                  elsif tag_name.present?
                    current_user.organization.services.tagged_with(tag_name).includes(:organization).order(
                      'organizations.name', :name
                    )
                  else
                    current_user.organization.services.includes(:organization).order('organizations.name', :name)
                  end
    ...
    def submit
      ...
      @service.submit
      if @service.save
        UserMailer.service_event_notification(subject: 'Service was submitted', service: @service, event: :submitted,
                                              link: admin_service_url(@service)).deliver_later
        redirect_to admin_service_path(@service), notice: 'Service was successfully updated.'
      end
    end
    ...
    def approve
      ...
      @service.approve
      if @service.save
        UserMailer.service_event_notification(subject: 'Service was approved', service: @service, event: :approved,
                                              link: admin_service_url(@service)).deliver_later
        redirect_to admin_service_path(@service), notice: 'Service was successfully updated.'
      end
    end
    ...
    def verify
      ...
      @service.verify
      if @service.save
        UserMailer.service_event_notification(subject: 'Service was activated', service: @service, event: :activated,
                                              link: admin_service_url(@service)).deliver_later
        redirect_to admin_service_path(@service), notice: 'Service was successfully updated.'
      end
    end
    ...
    def archive
      ...
      @service.archive
      if @service.save
        UserMailer.service_event_notification(subject: 'Service was archived', service: @service, event: :archived,
                                              link: admin_service_url(@service)).deliver_later
        redirect_to admin_service_path(@service), notice: 'Service was successfully updated.'
      end
    end
    ...
    def search
      ...
      if search_text.present?
        ...
        @services = Service.joins(:organization).where(' services.name ilike ? or organizations.name ilike ?  ',
                                                       search_text, search_text)
      elsif tag_name.present?
    ...
    def set_service_providers
      @service_providers = if service_manager_permissions?
                             ServiceProvider.all.includes(:organization).order('organizations.abbreviation',
                                                                               'service_providers.name')
                           else
                             current_user.organization.service_providers.includes(:organization).order(
                               'organizations.abbreviation', 'service_providers.name'
                             )
                           end
    end

    def set_service_owner_options
      @service_owner_options = if service_manager_permissions?
                                 User.active.order('email')
                               else
                                 current_user.organization.users.active.order('email')
                               end
