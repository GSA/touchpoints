  ...
  class ServicesController < AdminController
    ...
    def index
      ...
      @services = if service_manager_permissions?
                    ...
                    current_user.organization.services.tagged_with(tag_name).includes(:organization).order('organizations.name', :name)
                  else
    ...
    def set_service_providers
      @service_providers = if service_manager_permissions?
                             ServiceProvider.all.includes(:organization).order('organizations.abbreviation', 'service_providers.name')
                           else
                             current_user.organization.service_providers.includes(:organization).order('organizations.abbreviation', 'service_providers.name')
                           end
    end
