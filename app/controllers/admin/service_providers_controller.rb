...
module Admin
  class ServiceProvidersController < AdminController
    before_action :set_service_provider,
                  only: %i[show edit update destroy add_tag remove_tag add_service_provider_manager
    ...
    def index
      @service_providers = ServiceProvider.all.includes(:organization).order('organizations.name',
    ...
    def search
      ...
      if search_text.present?
        ...
        @service_providers = ServiceProvider.joins(:organization).where(
          ' service_providers.name ilike ? or organizations.name ilike ?  ', search_text, search_text
        )
    ...
    def add_service_provider_manager
      ...
      @manager.add_role(:service_provider_manager, @service) unless @manager.has_role?(:service_provider_manager,
                                                                                       @service)
    end
    ...
    def set_service_provider_manager_options
      @service_provider_manager_options = if service_manager_permissions?
