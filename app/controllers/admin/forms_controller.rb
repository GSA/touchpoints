...
module Admin
  class FormsController < AdminController
    ...
    def index
      @forms = if admin_permissions?
                 Form.non_templates.order('organization_id ASC').order('name ASC')
               else
                 current_user.forms.non_templates.order('organization_id ASC').order('name ASC').entries
               end
    end
    ...
    def invite
      ...
      if invitee.present? && invitee =~ URI::MailTo::EMAIL_REGEXP && (if ENV['GITHUB_CLIENT_ID'].present?
                                                                        true
                                                                      else
                                                                        User::APPROVED_DOMAINS.any? do |word|
                                                                          invitee.end_with?(word)
                                                                        end
                                                                      end)
    ...
    def publish
      Event.log_event(Event.names[:form_published], 'Form', @form.uuid, "Form #{@form.name} published at #{DateTime.now}",
                      current_user.id)
    ...
    def archive
      Event.log_event(Event.names[:form_archived], 'Form', @form.uuid, "Form #{@form.name} archived at #{DateTime.now}",
                      current_user.id)
    ...
    def reset
      Event.log_event(Event.names[:form_reset], 'Form', @form.uuid, "Form #{@form.name} reset at #{DateTime.now}",
                      current_user.id)
    ...
    def permissions
      ensure_form_manager(form: @form)
      @available_members = if admin_permissions?
                             User.all.order(:email) - @form.users
                           else
                             @form.organization.users.active.order(:email) - @form.users
                           end
    end
    ...
    def example
      if @form.delivery_method == 'touchpoints-hosted-only'
        redirect_to touchpoint_path,
                    notice: 'Previewing Touchpoint' and return
      end
      unless @form.delivery_method == 'modal' || @form.delivery_method == 'inline' || @form.delivery_method == 'custom-button-modal'
        redirect_to admin_forms_path,
                    notice: "Form does not have a delivery_method of 'modal' or 'inline' or 'custom-button-modal'" and return
      end
    ...
    def copy
      respond_to do |format|
        ...
        if new_form.valid?
          ...
          Event.log_event(Event.names[:form_copied], 'Form', @form.uuid, "Form #{@form.name} copied at #{DateTime.now}",
                          current_user.id)
    ...
    def update
      ...
      respond_to do |format|
        if @form.update(form_params)
          ...
          format.html do
            render (params[:form][:delivery_method].present? ? :delivery_method : :edit), status: :unprocessable_entity
          end
          format.json { render json: @form.errors, status: :unprocessable_entity }
        end
      end
    ...
    def destroy
      ...
      respond_to do |format|
        format.html do
          if @form.destroy
            Event.log_event(Event.names[:form_deleted], 'Form', @form.uuid,
                            "Form #{@form.name} deleted at #{DateTime.now}", current_user.id)
    ...
    def export_submissions
      ...
      respond_to do |format|
        ...
        format.json do
          ExportJob.perform_later(params[:uuid], @form.short_uuid, start_date.to_s, end_date.to_s,
                                  "touchpoints-form-responses-#{timestamp_string}.csv")
    ...
    def export_a11_header
      ...
      respond_to do |format|
        format.csv do
          send_data @form.to_a11_header_csv(start_date:, end_date:),
                    filename: "a11-header-#{timestamp_string}.csv"
        end
      end
    end
    ...
    def export_a11_submissions
      ...
      respond_to do |format|
        format.csv do
          send_data @form.to_a11_submissions_csv(start_date:, end_date:),
                    filename: "A-11-Responses-#{timestamp_string}.csv"
        end
      end
    end
    ...
    def transition_state
      if (params['form']['aasm_state'] == 'live') && !@form.live?
        Event.log_event(Event.names[:form_published], 'Form', @form.uuid,
                        "Form #{@form.name} published at #{DateTime.now}", current_user.id)
      end
      if (params['form']['aasm_state'] == 'archived') && !@form.archived?
        Event.log_event(Event.names[:form_archived], 'Form', @form.uuid,
                        "Form #{@form.name} archived at #{DateTime.now}", current_user.id)
      end
    end
