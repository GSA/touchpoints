...
module Admin
  class DigitalProductsController < AdminController
    ...
    def review
      @digital_products = DigitalProduct.where("aasm_state = 'created' OR aasm_state = 'edited'").order(:name,
    ...
    def submit
      if @digital_product.submit!
        Event.log_event(Event.names[:digital_product_submitted], 'Digital Product', @digital_product.id,
                        "Digital Product #{@digital_product.name} submitted at #{DateTime.now}", current_user.id)
    ...
    def publish
      ...
      if @digital_product.publish!
        Event.log_event(Event.names[:digital_product_published], 'Digital Product', @digital_product.id,
                        "Digital Product #{@digital_product.name} published at #{DateTime.now}", current_user.id)
        ...
        redirect_to admin_digital_product_path(@digital_product),
                    notice: "Digital Product #{@digital_product.name} was published."
      else
        render :edit
      end
    end
    ...
    def archive
      ...
      if @digital_product.archive!
        Event.log_event(Event.names[:digital_product_archived], 'Digital Product', @digital_product.id,
                        "Digital Product #{@digital_product.name} archived at #{DateTime.now}", current_user.id)
        redirect_to admin_digital_product_path(@digital_product),
                    notice: "Digital Product #{@digital_product.name} was archived."
      else
        render :edit
      end
    end
    ...
    def reset
      ...
      if @digital_product.reset!
        Event.log_event(Event.names[:digital_product_reset], 'Digital Service Account', @digital_product.id,
                        "Digital Product #{@digital_product.name} reset at #{DateTime.now}", current_user.id)
        redirect_to admin_digital_product_path(@digital_product),
                    notice: "Digital Service Account #{@digital_product.name} was reset."
      else
        render :edit
      end
    end
    ...
    def search
      ...
      @digital_products = if search_text && search_text.length >= 3
                            DigitalProduct.where("name ilike '%#{search_text}%'")
                          elsif organization_id
                            DigitalProduct.where(organization_id:)
                          else
                            DigitalProduct.all
                          end
    end
