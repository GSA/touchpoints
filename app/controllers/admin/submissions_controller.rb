...
module Admin
  class SubmissionsController < AdminController
    ...
    before_action :set_submission,
                  except: %i[feed export_feed search a11_chart a11_analysis responses_per_day responses_by_status performance_gov
                             submissions_table]
    ...
    def update
      respond_to do |format|
        if @submission.update(status_params)
          Event.log_event(Event.names[:response_status_changed], 'Submission', @submission.id,
                          "Submission #{@submission.id} changed status to #{status_params[:aasm_state]} at #{DateTime.now}", current_user.id)
    ...
    def search
      ...
      @submissions = if params[:archived]
                       @all_submissions.order('submissions.created_at DESC').page params[:page]
                     else
                       @all_submissions.non_archived.order('submissions.created_at DESC').page params[:page]
                     end
    end

    def flag
      Event.log_event(Event.names[:response_flagged], 'Submission', @submission.id,
                      "Submission #{@submission.id} flagged at #{DateTime.now}", current_user.id)
      @submission.update(flagged: true)
    end

    def unflag
      Event.log_event(Event.names[:response_unflagged], 'Submission', @submission.id,
                      "Submission #{@submission.id} unflagged at #{DateTime.now}", current_user.id)
    ...
    def responses_per_day
      ...
      @dates.each do |date|
        @response_groups << [date, 0] unless @response_groups.detect do |row|
                                               row[0].strftime('%m %d %Y') == date.strftime('%m %d %Y')
                                             end
      end
    ...
    def archive
      ...
      Event.log_event(Event.names[:response_archived], 'Submission', @submission.id,
                      "Submission #{@submission.id} archived at #{DateTime.now}", current_user.id)
    ...
    def unarchive
      ...
      Event.log_event(Event.names[:response_unarchived], 'Submission', @submission.id,
                      "Submission #{@submission.id} unarchived at #{DateTime.now}", current_user.id)
    ...
    def destroy
      ...
      Event.log_event(Event.names[:response_deleted], 'Submission', @submission.id,
                      "Submission #{@submission.id} deleted at #{DateTime.now}", current_user.id)
    ...
    def export_feed
      ...
      respond_to do |format|
        format.csv do
          send_data to_csv(@feed), type: 'text/csv; charset=utf-8; header=present',
                                   disposition: "attachment; filename=touchpoints-feed-#{Date.today}.csv"
        end
        format.json do
    ...
    def set_form
      @form = if admin_permissions?
                Form.find_by(short_uuid: params[:form_id])
              else
                current_user.forms.find_by(short_uuid: params[:form_id])
              end
