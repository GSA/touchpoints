  ...
  class UsersController < AdminController
    ...
    def index
      if current_user.admin? && params[:scope] == :all
        ...
        @users = User.all.includes(:organization).where('last_sign_in_at < ? OR last_sign_in_at ISNULL', 90.days.ago).order(
    ...
    def update
      respond_to do |format|
        if @user.update(user_params)
          Event.log_event(Event.names[:user_update], 'User', @user.id,
                          "User #{@user.email} was updated by #{current_user.email} on #{Date.today}")
    ...
    def destroy
      ...
      @user.destroy
      Event.log_event(Event.names[:user_deleted], 'User', @user.id,
                      "User #{@user.email} was deleted by #{current_user.email} on #{Date.today}")
      respond_to do |format|
    ...
    def deactivate
      ...
      user&.deactivate!
      Event.log_event(Event.names[:user_deactivated], 'User', user.id,
                      "User #{user.email} was deactivated on #{Date.today}")
