  ...
  class DigitalServiceAccountsController < AdminController
    ...
    def create
      ...
      if @digital_service_account.save
        ...
        Event.log_event(Event.names[:digital_service_account_created], 'Digital Service Account',
                        @digital_service_account.id, "Digital Service Account #{@digital_service_account.name} created at #{DateTime.now}", current_user.id)

        UserMailer.social_media_account_created_notification(digital_service_account: @digital_service_account,
                                                             link: admin_digital_service_account_path(@digital_service_account)).deliver_later
    ...
    def submit
      ...
      if @digital_service_account.submit!
        Event.log_event(Event.names[:digital_service_account_submitted], 'Digital Service Account',
                        @digital_service_account.id, "Digital Service Account #{@digital_service_account.name} submitted at #{DateTime.now}", current_user.id)
        ...
        redirect_to admin_digital_service_account_path(@digital_service_account),
                    notice: "Digital Service Account #{@digital_service_account.name} was submitted."
      else
        render :edit
      end
    end
    ...
    def publish
      ...
      if @digital_service_account.publish
        Event.log_event(Event.names[:digital_service_account_published], 'Digital Service Account',
                        @digital_service_account.id, "Digital Service Account #{@digital_service_account.name} published at #{DateTime.now}", current_user.id)
        ...
        redirect_to admin_digital_service_account_path(@digital_service_account),
                    notice: "Digital Service Account #{@digital_service_account.name} was published."
      else
        render :edit
      end
    end
    ...
    def archive
      ...
      if @digital_service_account.archive!
        Event.log_event(Event.names[:digital_service_account_archived], 'Digital Service Account',
                        @digital_service_account.id, "Digital Service Account #{@digital_service_account.name} archived at #{DateTime.now}", current_user.id)
        redirect_to admin_digital_service_account_path(@digital_service_account),
                    notice: "Digital Service Account #{@digital_service_account.name} was archived."
      else
        render :edit
      end
    end
    ...
    def reset
      ...
      if @digital_service_account.reset!
        Event.log_event(Event.names[:digital_service_account_reset], 'Digital Service Account',
                        @digital_service_account.id, "Digital Service Account #{@digital_service_account.name} reset at #{DateTime.now}", current_user.id)
        redirect_to admin_digital_service_account_path(@digital_service_account),
                    notice: "Digital Service Account #{@digital_service_account.name} was reset."
      else
        render :edit
      end
    end
    ...
    def search
      ...
      @digital_products = if search_text && search_text.length >= 3
                            DigitalServiceAccount.where("name ilike '%#{search_text}%'")
                          elsif organization_id
                            DigitalServiceAccount.where(organization_id:)
                          elsif service_text
                            DigitalServiceAccount.where("service ilike '%#{search_text}%'")
                          else
                            DigitalServiceAccount.all
                          end
    end
