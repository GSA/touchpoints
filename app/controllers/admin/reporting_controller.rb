...
module Admin
  class ReportingController < AdminController
    ...
    def hisp_services
      ...
      ServiceProvider.all.includes(:organization).order('organizations.name', :name).each do |service_provider|
        service_provider.services.order(:name).each do |service|
          service.omb_cx_reporting_collections.includes(:service, :collection).order('collections.year',
                                                                                     'collections.quarter', 'services.name').each do |omb_cx_reporting_collection|
            row_fields = [
    ...
    def hisp_service_cx_data_collections
      ...
      ServiceProvider.active.includes(:organization).order('organizations.name', :name).each do |service_provider|
        service_provider.services.includes(:organization).order('organizations.name', :name).each do |service|
          @collections = if params[:quarter]
                           service.collections.where(quarter: params[:quarter])
                         else
                           service.collections
                         end

          @collections.each do |collection|
            collection.omb_cx_reporting_collections.includes(:collection).order('collections.year',
                                                                                'collections.quarter', 'omb_cx_reporting_collections.service_provided', 'omb_cx_reporting_collections.channel').each do |omb_cx_reporting_collection|
              row_fields = [
                omb_cx_reporting_collection.organization_abbreviation,
                "\"#{omb_cx_reporting_collection.organization_name}\"",
                omb_cx_reporting_collection.organization_id,
    ...
    def lifespan
      ...
      @forms = Form.select(:id, :name, :organization_id,
                           :uuid).where('exists (select id, uuid from submissions where submissions.form_id = forms.id)')
    ...
    def no_submissions
      @forms = Form.live.select(:id, :name, :organization_id,
                                :uuid).where("not exists (select id, uuid from submissions where submissions.form_id = forms.id and submissions.created_at > current_date - interval '30' day)").order(:organization_id)
      @orgs = Organization.all.order(:name)
      @org_summary = []
      @orgs.each do |org|
        org_row = {}
