...
class Form < ApplicationRecord
  ...
  def check_expired
    if !archived? && expiration_date.present? && (expiration_date <= Date.today)
      archive!
      Event.log_event(Event.names[:form_archived], 'Touchpoint', id,
                      "Touchpoint #{name} archived on #{Date.today}")
    end
  end
  ...
  def touchpoints_js_string
    ApplicationController.new.render_to_string(partial: 'components/widget/fba', formats: :js,
                                               locals: { touchpoint: self })
  end

  def to_csv(start_date: nil, end_date: nil)
    non_flagged_submissions = submissions.non_flagged.where('created_at >= ?', start_date).where(
      'created_at <= ?', end_date
    ).order('created_at')
