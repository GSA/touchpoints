...
class FormCache
  ...
  def self.fetch(short_uuid)
    ...
    Rails.cache.fetch(NAMESPACE + short_uuid.to_s, expires_in: 1.day) do
      ...
      Form.includes([:questions, { form_sections: [questions: [:question_options]] }],
                    :organization).find_by(short_uuid:)
    end
  end
  ...
  def self.fetch_performance_gov_analysis(short_uuid)
    ...
    Rails.cache.fetch("#{NAMESPACE}-performance-gov-analysis-#{short_uuid}", expires_in: 1.day) do
      form = Form.find_by(short_uuid:)
      report = {}
      report[:quarterly_submissions] = form.submissions.order(:created_at).entries.map do |e|
        e.attributes.merge(quarter: e.created_at.beginning_of_quarter.to_date, end_of_quarter: e.created_at.end_of_quarter)
      end
